...
BOOL CFileLayer::Send(unsigned char *ppayload, int nlength, int port)
{
	...
	TRY
	{
		CFile m_fileDes(filename, CFile::modeRead);
		...
		if (m_fileDes.GetLength() > FILE_DATA_SIZE) {
			...
			while (m_fileDes.GetLength() > FILE_DATA_SIZE) {
				...
				///////////////////////set Time
				/*
				((CIPCAppDlg*) mp_aUpperLayer[0])->SetTimer(1, 1000, NULL);
				while (!AckSuccess);// {//waitting
				if (ResendPacket){
				mp_UnderLayer->Send((unsigned char*) &m_sHeader, sendLength + FILE_HEADER_SIZE, FILE_TYPE);
				ResendPacket = FALSE;
				((CIPCAppDlg*) mp_aUpperLayer[0])->SetTimer(1, 1000, NULL);
				...
				((CIPCAppDlg*) mp_aUpperLayer[0])->KillTimer(1);
				*/
				...
				if (fileInfo.totalLength - totalLength <= FILE_DATA_SIZE) {
					...
					////////////////////////////////////////////////////////////////////////////
					CString message;
					message.Format("Count >> %d", m_sHeader.fapp_seq_num);//sended packet's num
					int index = ((CIPCAppDlg*) mp_aUpperLayer[0])->m_ListChat.AddString(message);
					((CIPCAppDlg*) mp_aUpperLayer[0])->m_ListChat.SetCurSel(index);
					////////////////////////////////////////////////////////////////////////////
					...
					RecWait = FALSE;//we can use time-out system(but, this is not used yet..)
				}

				while (RecWait);//out while loop
...
///////////////////////////////////////we can use time-out system(but, this is not used yet..)
/*
void CFileLayer::OnTimer(UINT nIDEvent)//regack를 못 받으면 출력하는 함수
{
ResendPacket = TRUE;//
((CIPCAppDlg*) mp_aUpperLayer[0])->KillTimer(1);

CFileLayer::OnTimer(nIDEvent);
}
*/